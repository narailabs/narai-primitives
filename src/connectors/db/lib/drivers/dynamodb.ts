...
export class DynamoDriver extends DatabaseDriver {
  ...
  async getSchemaAsync(
    ...
    try {
      ...
      // ⚡ Bolt Optimization: Use chunked concurrent requests to avoid N+1 sequential latency
      // while preventing AWS API rate limits. N+1 latency scales linearly with tables,
      // whereas chunking bounds the latency.
