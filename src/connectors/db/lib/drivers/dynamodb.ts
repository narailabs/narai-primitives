...
export class DynamoDriver extends DatabaseDriver {
  ...
  async getSchemaAsync(
    ...
    try {
      ...
      // ⚡ Bolt Optimization: Chunked concurrent DescribeTable requests.
      // Avoids N+1 sequential latency while respecting AWS API rate limits.
      const out: Table[] = [];
