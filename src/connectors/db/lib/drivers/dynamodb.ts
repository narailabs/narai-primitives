...
export class DynamoDriver extends DatabaseDriver {
  ...
  async getSchemaAsync(
    ...
    try {
      ...
      const out: Table[] = [];
      // ⚡ Bolt Optimization: Parallelize DescribeTable requests in chunks to avoid N+1 sequential latency.
      // We chunk requests (e.g., 10 at a time) to parallelize network I/O while avoiding AWS API throttling.
      const CHUNK_SIZE = 10;
