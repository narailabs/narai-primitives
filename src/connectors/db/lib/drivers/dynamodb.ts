...
export class DynamoDriver extends DatabaseDriver {
  ...
  async getSchemaAsync(
    ...
    try {
      ...
      // ⚡ Bolt Optimization: Filter table names first, then manually chunk concurrent requests.
      // This avoids N+1 query latency by running `DescribeTableCommand` in parallel
      // batches of 10. `p-limit` is not available, so we chunk manually.
      // This avoids AWS API throttling exceptions while drastically reducing latency.
