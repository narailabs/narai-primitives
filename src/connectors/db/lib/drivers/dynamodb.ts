...
export class DynamoDriver extends DatabaseDriver {
  ...
  async getSchemaAsync(
    ...
    try {
      ...
      // ⚡ Bolt Optimization: Fetch table schemas concurrently in chunks to avoid N+1
      // network latency and AWS API throttling exceptions.
      ...
      for (let i = 0; i < filteredNames.length; i += concurrencyLimit) {
        chunks.push(filteredNames.slice(i, i + concurrencyLimit));
      }
      ...
      for (const chunk of chunks) {
        ...
        for (let i = 0; i < chunk.length; i++) {
          const name = chunk[i]!;
          const desc = descriptions[i] as DescribeTableOutput;
