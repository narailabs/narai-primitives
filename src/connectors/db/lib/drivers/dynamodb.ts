...
export class DynamoDriver extends DatabaseDriver {
  ...
  async getSchemaAsync(
    ...
    try {
      ...
      // ⚡ Bolt Optimization: Fetch table schemas in chunks concurrently
      // This avoids N+1 sequential network requests for DescribeTableCommand
      // while preventing AWS API throttling that occurs with unbounded Promise.all().
