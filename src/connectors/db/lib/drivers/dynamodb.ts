...
export class DynamoDriver extends DatabaseDriver {
  ...
  async getSchemaAsync(
    ...
    try {
      ...
      // ⚡ Bolt Optimization: Batch DescribeTable commands
      // DescribeTable is called in a loop, causing sequential N+1 network delay.
      // We chunk requests to parallelize them without hitting AWS throttling limits.
      const CHUNK_SIZE = 10;
