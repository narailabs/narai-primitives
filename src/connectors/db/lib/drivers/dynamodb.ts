...
export class DynamoDriver extends DatabaseDriver {
  ...
  async getSchemaAsync(
    ...
    try {
      ...
      const filteredNames =
        filterRe !== null ? names.filter((n) => filterRe.test(n)) : names;

      const out: Table[] = [];

      // ⚡ Bolt Optimization: Chunked concurrent requests
      // Describing tables sequentially creates an N+1 network bottleneck.
      // Using Promise.all with a small chunk size (e.g., 5) parallelizes
      // the network calls while avoiding AWS API ThrottlingExceptions.
      const CHUNK_SIZE = 5;
      for (let i = 0; i < filteredNames.length; i += CHUNK_SIZE) {
        ...
        for (let j = 0; j < descriptions.length; j++) {
          ...
          out.push(
            new Table({ name: name as string, schema: schemaName, columns }),
          );
        }
      }
