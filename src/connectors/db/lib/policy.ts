...
/**
 * ⚡ Bolt Optimization: Uses an O(N) array instead of a Set to collect boundaries.
 * Since the loop naturally iterates sequentially, boundaries are already monotonically
 * increasing, eliminating the need for an O(N log N) `[...set].sort()` step later.
 */
