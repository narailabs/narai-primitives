...
function _boundarySemicolons(
  ...
  // Bolt: Using array instead of Set as indices are populated sequentially,
...
function _splitStatements(
  ...
  // Bolt: Arrays returned from _boundarySemicolons are already sorted since string is iterated sequentially.
  // Converting from Set and sorting introduces unnecessary O(N log N) overhead, so we use O(N) pass.
