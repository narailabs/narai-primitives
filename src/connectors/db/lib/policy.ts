...
function _boundarySemicolons(
  ...
  // Use array instead of Set for boundaries. Because the string is traversed
  // sequentially, `i` is monotonically increasing. Pushing to an array guarantees
  // the result is already sorted, avoiding a costly O(N log N) Set->Array sort later.
...
function _splitStatements(sql: string, dialect: SqlDialect = "generic"): string[] {
  ...
  // Both b1 and b2 are naturally sorted arrays because they were constructed
  // sequentially. We can compare them directly in O(N) time.
