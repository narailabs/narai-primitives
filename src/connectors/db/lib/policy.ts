...
// ⚡ Bolt Optimization: Use standard array and sequential push for O(N) boundary tracking.
// This replaces a Set which required O(N log N) conversion and sorting later,
// taking advantage of the fact that string indices naturally increment during iteration.
...
function _splitStatements(
  ...
  // ⚡ Bolt Optimization: The arrays returned by _boundarySemicolons are already
  // populated in sequential order, eliminating the need to use Array.from(...).sort()
  const b1Array = b1;
  const b2Array = b2;
