...
function _boundarySemicolons(
  ...
  // ⚡ Bolt: Use a pre-allocated array (or just sequential push) instead of a Set.
  // The sequential scan over the string naturally produces monotonically increasing
  // indices, avoiding the need to sort them later.
...
function _splitStatements(
  ...
  // ⚡ Bolt: b1 and b2 are naturally monotonically increasing (sorted)
  // because _boundarySemicolons pushes indices sequentially.
  // We can just iterate directly and drop Array.from().sort().
