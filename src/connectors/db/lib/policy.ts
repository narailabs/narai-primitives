...
function _boundarySemicolons(
  ...
  // ⚡ Bolt: Use a standard array populated sequentially via `.push()` instead of
  // Set tracking followed by Array.from().sort(). Bypasses hashing overhead and
  // redundant O(N log N) sorting since iteration naturally yields monotonic indices.
