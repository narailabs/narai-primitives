...
// ⚡ Bolt: Replace Set<number> with number[] since array iteration naturally pushes indices in monotonic (sorted) order.
// This avoids an unnecessary O(N log N) Set->Array conversion and sort step downstream.
...
function _splitStatements(
  ...
  // ⚡ Bolt: _boundarySemicolons now returns natively sorted arrays (number[]) instead of Sets,
  // avoiding the overhead of Array.from(...).sort(...) conversion.
