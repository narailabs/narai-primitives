...
function _boundarySemicolons(
  ...
  // ⚡ Bolt: Use simple array instead of Set. Since we loop sequentially,
  // indices are naturally monotonically increasing and sorted.
...
function _splitStatements(
  ...
  // ⚡ Bolt: Removed Array.from(...).sort(...) since _boundarySemicolons now
  // returns an already-sorted array, skipping O(N log N) overhead.
