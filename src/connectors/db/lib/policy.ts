...
// ⚡ Bolt: Replace Set with array for naturally monotonically increasing sequence
...
function _splitStatements(sql: string, dialect: SqlDialect = "generic"): string[] {
  ...
  // ⚡ Bolt: _boundarySemicolons naturally produces sorted array via sequential push,
  // avoiding O(N log N) Set->Array conversion and sort overhead.
  const b1Array = b1;
  const b2Array = b2;
