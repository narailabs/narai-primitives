...
function _splitStatements(sql: string, dialect: SqlDialect = "generic"): string[] {
  ...
  // _boundarySemicolons returns an array populated sequentially in O(N) time.
  // Because string indices naturally monotonically increase during parsing, the resulting
  // array is already sorted and distinct. We bypass Set conversion and O(N log N) sorting.
