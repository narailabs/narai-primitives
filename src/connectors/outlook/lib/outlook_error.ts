/**
 * outlook_error.ts — Microsoft Graph error envelope for the Outlook connector.
 *
...
/**
 * OutlookError is a `ConnectorError` whose `code` field is a Microsoft Graph
 ...
 * effectively a typed alias so callers can write `throw new OutlookError(...)`
 * without dragging the longer `ConnectorError` import everywhere.
 */
export class OutlookError extends ConnectorError {
  constructor(
    code: string,
    message: string,
    retriable: boolean,
