...
## 2024-06-12 - DynamoDB Schema Extraction Parallelization
**Learning:** Calling `DescribeTableCommand` sequentially in a loop for DynamoDB schema extraction creates an N+1 performance bottleneck over the network.
**Action:** Use chunked concurrent requests (`Promise.all` with a chunk size of 10) to parallelize network calls while avoiding AWS API throttling exceptions.
