...
## 2024-05-15 - Array push vs Set conversion for sorted sequential indices
**Learning:** When tracking index positions sequentially across a string loop, the indices naturally increment. Using a `Set` and then calling `Array.from(set).sort((a,b) => a-b)` incurs unnecessary memory allocations and O(N log N) sorting overhead for indices that were already strictly monotonic.
**Action:** Always collect sequential indices directly into a `number[]` array with `.push()` to bypass redundant allocations and the O(N log N) `.sort()` operation.
