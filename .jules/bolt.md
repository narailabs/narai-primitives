...
## 2024-06-04 - DynamoDB Schema Extraction Chunking
**Learning:** In DynamoDB, schema extraction using `DescribeTableCommand` within a `for...of` loop creates an N+1 query bottleneck. Because DynamoDB is HTTP-based, these sequential network requests result in high latency.
**Action:** Use `Promise.all` to batch the HTTP requests concurrently. To avoid AWS API rate limit throttling, chunk the tables array into smaller batches (e.g., 10 at a time) and await each chunk's `Promise.all` result.
