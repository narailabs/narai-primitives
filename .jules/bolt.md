...
## 2024-05-24 - Array vs Set in Sequential Parsers\n**Learning:** When tracking index positions sequentially in a loop (like parsing boundaries), accumulating into a Set and later calling Array.from().sort() is redundant and slow. Since indices are appended monotonically, a plain array handles it instantly in O(N).\n**Action:** Always prefer standard arrays over Sets for sequentially discovered indices to avoid O(N log N) conversion bottlenecks.
