...
## 2024-05-29 - Parallelize DynamoDB Schema Extraction
**Learning:** Sequential network calls for schema extraction in DynamoDB can be a bottleneck. While relational databases can batch queries in SQL, DynamoDB requires batching the promises.
**Action:** Use `Promise.all` with chunking to parallelize `DescribeTableCommand` calls, avoiding rate limits while improving performance.
