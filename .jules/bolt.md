...
## 2026-09-06 - SQL Boundary Parsing Array Optimization
**Learning:** During string iteration where index naturally increases sequentially, using `Set` and subsequently converting to array and sorting adds unnecessary CPU and memory overhead.
**Action:** Use a sequentially-populated standard array (`number[]`) and `push` operation instead, resulting in naturally sorted outputs with lower overhead.
