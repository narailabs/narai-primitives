...
## 2024-07-28 - Set to Array for Sequential Loops
**Learning:** Using `Set` to collect sequentially increasing indices and then later converting to an array and sorting (`Array.from(set).sort()`) introduces completely unnecessary memory allocation and an O(N log N) performance hit.
**Action:** When tracking index positions sequentially in string or array processing (like SQL parsing), push directly to a standard array. The output is naturally sorted and unique.
