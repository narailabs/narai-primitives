...
## 2026-07-19 - Fast Sequential Set/Sort Replacement
**Learning:** In the SQL parser (`_boundarySemicolons`), tracking index positions linearly using `Set` followed by `Array.from().sort()` causes severe O(N log N) overhead.
**Action:** When iterating strings/arrays sequentially without backtracking, use standard arrays (`[]`) and `push()`. The resulting array is implicitly sorted without any extra overhead.
