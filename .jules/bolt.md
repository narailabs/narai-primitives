...
## 2024-05-19 - Set vs Array for Monotonic Append
**Learning:** In sequential parsing tasks (like SQL boundary detection), using a `Set<number>` for uniqueness is unnecessary and incurs O(N log N) overhead if it's subsequently converted to an array and sorted. When iterating linearly from left-to-right, an array naturally stays sorted.
**Action:** Use a `number[]` and push values directly when collecting sequential indices in parsing algorithms to ensure O(N) performance.
