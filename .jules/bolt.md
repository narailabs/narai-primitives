...
## 2026-08-25 - Array vs Set in Sequential Loops\n**Learning:** When tracking index positions sequentially in a loop (like parsing boundaries), pushing to an array and using it directly is much faster than adding to a Set and then converting/sorting it, avoiding unnecessary O(N log N) overhead.\n**Action:** Use standard arrays for tracking sequential indices where uniqueness and order are naturally guaranteed by the loop structure.
