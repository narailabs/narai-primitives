...
## 2026-06-03 - SQL Parsing: Avoid Sets for Sequential Discovery
**Learning:** During the parsing of SQL strings for boundaries (e.g., semicolons), using a `Set` to deduplicate indices introduces measurable overhead during both collection (`Set.add`) and retrieval (`Array.from(set).sort(...)`). Because a string is traversed sequentially (`for (let i = 0...)`), the discovered indices are intrinsically unique and sorted.
**Action:** Use a plain array (`number[]`) with `.push()` instead of a `Set` when extracting boundary indices sequentially. This drops the time complexity of retrieval from O(N log N) to O(N).
