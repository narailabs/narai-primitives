...
## 2026-08-21 - Optimize array extraction over Set

**Learning:** When collecting naturally sequential index boundaries during a string traversal, using `Set<number>` introduces unnecessary overhead and O(N log N) sorting later when converting to arrays.
**Action:** Use a standard `number[]` array and `.push()` for natively tracking monotonically increasing indexes instead.
