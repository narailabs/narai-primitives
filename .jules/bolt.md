...
## 2024-10-24 - DynamoDB Concurrent Schema Extraction
**Learning:** Like relational databases, DynamoDB schema extraction suffers from N+1 query problems when calling `DescribeTableCommand` sequentially in a loop.
**Action:** Since DynamoDB cannot resolve `DescribeTableCommand` with a batched query across multiple tables, use concurrent chunked requests with `Promise.all` (chunk size ~10) to parallelize network calls while preventing AWS API rate limiting exceptions.
