...
## 2026-06-29 - Regex Unrolling Optimization
**Learning:** Using loop unrolling (`[^'\\]*(?:\.[^'\\]*)*`) instead of alternation (`(?:[^'\\]|\.)*`) provides linear performance without introducing catastrophic backtracking (ReDoS) vulnerability.
**Action:** Use loop unrolling instead of alternation for quoted string matching.
