...
## 2024-05-31 - DynamoDB Schema Parallelization
**Learning:** Extracting schema for DynamoDB table by table sequentially via `DescribeTableCommand` leads to an N+1 API call bottleneck.
**Action:** Use chunked concurrent requests via `Promise.all` with a concurrency limit. This dramatically speeds up metadata extraction for databases with many tables while safely avoiding AWS API throttling exceptions.
