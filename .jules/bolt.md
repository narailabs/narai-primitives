...
## 2024-07-28 - O(N log N) sorting for monotonically increasing loop iteration
**Learning:** Sequential loops that track string indices (like parsing boundaries) naturally produce monotonically increasing arrays. Pushing to a standard `number[]` array is optimal. Collecting items in a `Set` and then calling `Array.from(...).sort()` adds redundant and slow O(N log N) sorting overhead for data that is already sorted by nature.
**Action:** Use standard arrays and `push` when tracking sequentially discovered string index boundaries, and avoid `Set` and `.sort()` on them unless deduplication/out-of-order data is possible.
