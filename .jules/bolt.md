...
## 2026-05-27 - Remove Redundant Index Sorting
**Learning:** In `policy.ts`, using a `Set<number>` and then `Array.from(...).sort(...)` to track semicolon indices introduces O(N log N) overhead. Because the string is scanned linearly, the discovered indices are naturally sorted.
**Action:** Push boundary indices directly into a `number[]` array to avoid Set allocation and sorting overhead in inherently sequential operations.
