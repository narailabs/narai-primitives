...
Lives at `<workspace>/db-agent-connector/`. Single CLI surface, single library API, single Claude Code plugin — but speaks to **seven** database systems through a pluggable driver registry:
...
2. **Confirm db-agent is installed**: check whether `<workspace>/db-agent-connector/` has `node_modules/`. If not, `cd` there and run `npm install`.
...
- `<workspace>/db-agent-connector/CLAUDE.md` — authoritative documentation for db-agent.
- `<workspace>/db-agent-connector/src/connector.ts` — the policy-gate orchestration.
- `<workspace>/db-agent-connector/src/lib/policy.ts` — the SQL keyword classifier.
- `<workspace>/db-agent-connector/src/lib/plugin_config.ts` — config schema.
- `<workspace>/db-agent-connector/src/lib/drivers/` — driver registry pattern (for extension).
