...
- **Date**: 2026-04-26
- **Source connector**: `<workspace>/notion-agent-connector/`
...
2. **Diff the canonical against the template** (treating placeholders as free variables):
   ```bash
   diff -u \
     <(sed 's/notion/{{SERVICE_SLUG}}/g; s/Notion/{{ServicePascal}}/g; s/NOTION/{{SERVICE_SLUG_UPPER}}/g' <workspace>/notion-agent-connector/<file>) \
     <skill-root>/assets/templates/connector/<file>.tmpl
   ```
