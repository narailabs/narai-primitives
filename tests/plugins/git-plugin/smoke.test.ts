/**
 * smoke.test.ts — end-to-end test of the git-plugin gates via the shared
 * dispatcher. Spawns dispatcher.mjs as a subprocess with the real
 * `plugins/git-plugin/` directory as CLAUDE_PLUGIN_ROOT so plugin-config.json
...
const PLUGIN_ROOT = path.join(REPO_ROOT, "plugins", "git-plugin");
