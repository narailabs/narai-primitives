...
describe("usage-report CLI", () => {
  ...
  it("never echoes a rejected --format value, credential-shaped or bare", () => {
    ...
    for (const secret of ["password='s3cret'", "hunter2", "ghp_AAAABBBBCCCC"]) {
      ...
      expect(r.status).toBe(2);
      expect(r.stderr).toContain("--format must be 'json' or 'md'");
  ...
  it("never echoes a rejected argument token", () => {
    ...
    const r = run([secret]);
    expect(r.status).toBe(2);
    // Pin the rejection MESSAGE, not the help text: HELP is written to stderr
    // on this path too and already names every flag, so asserting on
    // "--connector" alone would pass with the accepted set removed.
    expect(r.stderr).toContain(
      "Unknown argument (expected --connector, --since, --format, --dir, --help)",
    );
    expect(r.stderr).not.toContain(secret);
