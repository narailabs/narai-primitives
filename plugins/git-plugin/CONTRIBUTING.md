...
echo '{"tool_name":"Bash","tool_input":{"command":"git push origin main"},"hook_event_name":"PreToolUse"}' \
  | CLAUDE_PLUGIN_ROOT=plugins/git-plugin \
    CLAUDE_PLUGIN_DATA=/tmp/git-plugin-data \
...
2. Add a `describe` block in [`tests/plugins/git-plugin/gates.test.ts`](../../tests/plugins/git-plugin/gates.test.ts) covering:
...
3. Add an end-to-end case in [`tests/plugins/git-plugin/smoke.test.ts`](../../tests/plugins/git-plugin/smoke.test.ts) if the rule covers a category not already exercised.
...
- Run `npx vitest run tests/plugins/git-plugin/` before pushing. The full repo suite (`npx vitest run`) should also pass — the plugin shares the dispatcher with every connector, so dispatcher-touching changes need the broader suite green.
