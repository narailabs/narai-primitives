# Task 3: Relocate read actions into `actions/reads.ts`

Goal: Extract the 9 existing read action specs from `src/connectors/github/index.ts` into a new module `src/connectors/github/actions/reads.ts` exposing a `buildReadActions(deps)` factory. `index.ts` then imports and spreads them. This is the first "actions module" in the new layout — it establishes the pattern that subsequent tasks (PRs, issues, comments, releases, workflows) follow.

**Files:**
- Create: `src/connectors/github/actions/reads.ts`
- Modify: `src/connectors/github/index.ts` (remove the 9 inline action specs, import + spread from `reads.ts`)
- Test: existing `tests/connectors/github/integration/framework.test.ts` and `tests/connectors/github/unit/cli.test.ts` continue to pass — no test changes in this task

- [ ] **Step 1: Snapshot the existing action surface so we can assert no regression**
...
- [ ] **Step 2: Create the action-module shared types file used by every domain**

Create `src/connectors/github/actions/_types.ts`:
...
export type GithubActions = Record<string, ActionSpec<unknown, GithubClient>>;
...
- [ ] **Step 3: Create `actions/reads.ts` with the 9 lifted action specs**

Create `src/connectors/github/actions/reads.ts`. Paste the body below verbatim — it is the 9 read action specs from the current `index.ts`, plus the helper `paginate` that they all share, plus their zod schemas. Note: `GithubActionDeps` is accepted but unused for reads — kept for symmetry with the other domains.
...
- [ ] **Step 4: Strip the 9 inline action specs from `index.ts` and wire the import**

Open `src/connectors/github/index.ts`. Replace the entire `actions: { ... }` map inside `createConnector<GithubClient>({...})` with a spread of `buildReadActions(...)`. Add the import. After this step (Tasks 4-8 will add more spreads alongside), the relevant slice of `index.ts` looks like this:

```ts
import { loadGithubBehavior } from "./lib/github_config.js";
import { buildReadActions } from "./actions/reads.js";
// ... existing imports for createConnector, ConnectorError, etc.

export function buildGithubConnector(overrides: BuildOptions = {}): Connector {
  // ... existing defaultCredentials and defaultSdk

  const behavior = loadGithubBehavior();

  return createConnector<GithubClient>({
    name: "github",
    version: "3.0.1",                   // version bump deferred to Task 13
    scope: githubScope,
...
Also remove the now-orphaned schema constants (`MAX_RESULTS_DEFAULT`, `MAX_RESULTS_CAP`, `GITHUB_MAX_PER_PAGE`, `ownerRepoField`, `repoInfoParams`, `searchCodeParams`, `VALID_STATES`, `getIssuesParams`, `getPullsParams`, `getFileParams`, `issueNumberField`, `getIssueCommentsParams`, `getPrReviewCommentsParams`, `tagField`, `listReleaseAssetsParams`, `getReleaseAssetParams`) and the `paginate` helper — they have moved to `reads.ts`. Keep: `ConnectorError`, `CODE_MAP`, `githubScope`, `BuildOptions`, `buildGithubConnector`, default export, and re-exports.
...
- [ ] **Step 6: Run github tests — confirm no regressions**

Run:
```
npx vitest run tests/connectors/github
```
Expected: same passing count as before plus the unchanged 13 from Task 2 and 5 from Task 1. No new failures.
...
```
git add src/connectors/github/actions/_types.ts src/connectors/github/actions/reads.ts src/connectors/github/index.ts
git commit -m "refactor(github): lift read actions into actions/reads.ts module

Pure refactor — the 9 read actions and their pagination helper move
out of index.ts into a buildReadActions() factory under
src/connectors/github/actions/reads.ts. index.ts now imports + spreads
the result. Establishes the per-domain action-module pattern that
PRs, issues, comments, releases, and workflows will follow."
```
